...
export interface MailContextHandlers {
  ...
  createContactFromSender?: (message: MailListItem) => void

  openSenderContact?: (contactId: number) => void
...
export interface MailContextMenuUi {
  ...
  /** Bereits gespeicherter Kontakt zum Absender (Kontextmenue). */
...
export function buildMailContextItems(
  ...
  return [
    ...
    ...(msg.fromAddr?.trim() && (h.createContactFromSender || h.openSenderContact)
      ? [{ id: 'sep-sender-contact', label: '', separator: true }]
      : []),
