...
export type DashboardCatalogTile = {
  id: DashboardTileId
  title: string
  subtitle?: string
  icon?: LucideIcon
...
export function useDashboardTileCatalog(opts?: {
  /** Nur auf Start: Posteingangs-Vorschau für die Inbox-Kachel vorbereiten. */
  prepareInboxPreview?: boolean
}): {
  tiles: DashboardCatalogTile[]
  tileById: Map<string, DashboardCatalogTile>
  getCustomTileBody: (entry: DashboardCustomTileStored) => React.ReactNode
  customWizardCalendarEvents: CalendarEventView[]
  contextMenu: MailContextState | null
  setContextMenu: (v: MailContextState | null) => void
  ...
  const tileById = useMemo(
    () => new Map(dashboardTiles.map((tile) => [tile.id, tile] as const)),
    [dashboardTiles]
  )
