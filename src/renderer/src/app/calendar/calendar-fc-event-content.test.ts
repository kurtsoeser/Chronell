...
describe('calendarFcEventContent', () => {
  it('Monatsansicht: kleines Icon vor der Uhrzeit, Titel darunter', () => {
    ...
    const timeRow = root.querySelector('.fc-cal-event-custom-time-row')
    expect(timeRow).not.toBeNull()
    expect(timeRow?.querySelector('.fc-cal-event-custom-time')?.textContent).toBe('09:30 - 10:00')
    const inlineIcon = timeRow?.querySelector('.fc-cal-event-kind-icon--inline')
  ...
  it('Wochenansicht: Icon inline vor Uhrzeit, kein Absolut-Icon', () => {
    const { domNodes } = calendarFcEventContent(
      {
        event: {
          id: '1',
    ...
    const root = domNodes[0] as HTMLElement
    expect(root.classList.contains('fc-cal-event-custom--timegrid')).toBe(true)
    const timeRow = root.querySelector('.fc-cal-event-custom-time-row')
    expect(timeRow).not.toBeNull()
    expect(timeRow?.querySelector('.fc-cal-event-kind-icon--inline')).not.toBeNull()
    expect(root.querySelector(':scope > .fc-cal-event-kind-icon')).toBeNull()
  })
  ...
  it('zeigt Standard-Kalender-Icon ohne explizites Termin-Icon', () => {
    ...
    const root = domNodes[0] as HTMLElement
    const timeRow = root.querySelector('.fc-cal-event-custom-time-row')
    expect(timeRow?.querySelector('.fc-cal-event-kind-icon--inline')?.tagName.toLowerCase()).toBe(
      'svg'
    )
  })
})
