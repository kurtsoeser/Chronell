...
import { safeSetNoteCategories } from '@/lib/notes-ipc-client'
import { Check, Loader2, X } from 'lucide-react'

export interface NoteCategoriesPopoverProps {
  open: boolean
...
export function NoteCategoriesPopover({
  open,
  anchor,
  noteId,
  account,
  ...
  onClose,
  onSaved
}: NoteCategoriesPopoverProps): JSX.Element | null {
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        <span className="font-semibold text-foreground">Outlook-Kategorien</span>
        <button
          type="button"
          onClick={onClose}
      ...
      <p className="mb-2 leading-relaxed text-[10px] text-muted-foreground">
        Dieselben Kategorien wie in Mail, Kalender und Aufgaben. Bei Microsoft-Konten aus der
        Outlook-Masterliste.
      </p>
            ...
            return (
              <li key={name}>
                ...
                >
                  ...
                  {on ? <Check className="h-3.5 w-3.5 shrink-0 text-primary" aria-hidden /> : null}
                </button>
              </li>
            )
          })}
